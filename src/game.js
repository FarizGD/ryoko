...
const clock = new RhythmClock(chart.bpm), input = new InputManager(), editor = new ChartEditor(chart, clock);
...
function move(dir){const v={up:[0,-1],right:[1,0],down:[0,1],left:[-1,0]},[x,y]=v[dir];player.style.transform=`translate(calc(-50% + ${x*55}px),calc(-50% + ${y*55}px))`;setTimeout(()=>player.style.transform='translate(-50%,-50%)',100);judge(dir);}
function judge(dir){const now=clock.current;let best=null,bestAbs=Infinity;chart.notes.forEach((n,i)=>{if(i<=lastJudged||n.direction!==dir||n._hit)return;const d=Math.abs(n.time-now);if(d<bestAbs){best=n;bestAbs=d;}});if(!best||bestAbs>timing.good){combo=0;health=Math.max(0,health-1);setFeedback('MISS');updateHud();healthEl.classList.add('hit');setTimeout(()=>healthEl.classList.remove('hit'),200);return;}best._hit=true;lastJudged=Math.max(lastJudged,chart.notes.indexOf(best));combo++;const q=bestAbs<=timing.perfect?'PERFECT':bestAbs<=timing.great?'GREAT':'GOOD';score+=q==='PERFECT'?1000:q==='GREAT'?700:400;setFeedback(q);updateHud();}
function loop(){if(running){const now=clock.current;timeEl.textContent=formatTime(now);editor.updatePlayhead(now);}requestAnimationFrame(loop);}
...
input.onDirection=move;window.addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();$('start').click();}});editor.bindUI();updateHud();loop();
